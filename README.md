...
- Backend tạo prompt tiếng Việt, gửi ảnh upload/ảnh gần nhất trong chat tới Hugging Face Inference API và lưu cuộc hội thoại vào MySQL.
...
| `HF_MODEL_URL` | FLUX.1 Kontext image-to-image endpoint | Model endpoint hỗ trợ image-to-image |
| `HF_MOCK_ENABLED` | `false` | `true` ép dùng ảnh demo, `false` gọi Hugging Face khi có `HF_TOKEN` |
| `CORS_ALLOWED_ORIGINS` | `http://localhost:5173` | Origin frontend |

> Không commit Hugging Face access token vào source code. Hãy truyền token bằng biến môi trường `HF_TOKEN`. Backend chỉ dùng ảnh demo/sample khi bạn đặt `HF_MOCK_ENABLED=true`; nếu `HF_MOCK_ENABLED=false` mà thiếu `HF_TOKEN`, API sẽ trả lỗi cấu hình để tránh nhầm là kết quả AI thật. Với model image-to-image, backend gửi một ảnh tham chiếu dạng contact sheet được ghép từ ảnh upload ban đầu và ảnh gần nhất trong đoạn chat.
...
Nếu chỉ muốn chạy thử giao diện bằng ảnh demo, backend sẽ trả lại ảnh tham chiếu gần nhất thay vì gọi Hugging Face:

```bash
cd backend
HF_MOCK_ENABLED=true mvn spring-boot:run
```

...
> Không commit Hugging Face access token vào source code. Hãy truyền token bằng biến môi trường `HF_TOKEN`. Nếu chưa có token, backend tự chuyển sang chế độ demo; nếu muốn ép demo, đặt `HF_MOCK_ENABLED=true`.

...
> Không commit Hugging Face access token vào source code. Hãy truyền token bằng biến môi trường `HF_TOKEN`. Backend chỉ dùng ảnh demo/sample khi bạn đặt `HF_MOCK_ENABLED=true`; nếu `HF_MOCK_ENABLED=false` mà thiếu `HF_TOKEN`, API sẽ trả lỗi cấu hình để tránh nhầm là kết quả AI thật.
...
Nếu chỉ muốn chạy thử giao diện bằng ảnh demo/sample cố định:
